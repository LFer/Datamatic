...
class solicitud_recursos(osv.osv):
    ...
    def create(self, cr, uid, values, context=None):
         ...
         return super(solicitud_recursos, self).create(cr, uid, values, context=context)
...
#        'origin': fields.char ( 'Documento orígen', size = 32 )
        ...
        'state': fields.selection ( LISTA_ESTADOS_SOLICITUD, 'Estado', size = 20),
...
class solicitud_recursos_line ( osv.osv ):
    ...
    def onchange_product_id(self, cr, uid, ids, product_id, product_uom_id, context=None):
                ...
                if product_id:
                        ...
                        value = {'product_uom_id': prod.uom_id.id,'product_requested_qty':1.0,'precio':prod.standard_price}
                return {'value': value}
    ...
    _columns = {
        'product_id': fields.many2one ('product.template', 'Producto', required=True ), # Producto
        'product_uom_id': fields.many2one('product.uom', 'UdM'),
        ...
        'state': fields.related ( 'sr_solicitud_id', 'state', type = 'char'),
        'precio': fields.float('Precio'),
        'product_date_need':fields.date('Fecha de necesidad'),
        'product_purchase_order':fields.char('Orden de compra'),
        'comentarios': fields.text ( 'Comentarios', size = 30 ),
        'estado': fields.selection ( LISTA_ESTADOS_LINEA_SOLICITUD, 'Status entrega', size = 20),

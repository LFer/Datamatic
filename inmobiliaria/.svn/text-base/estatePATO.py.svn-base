...
class estate(osv.osv):
    ...
    def _prueba(self, cr, uid, ids, name, args, context=None):
        result = {}
        for record in self.browse(cr, uid, ids):
            for recordAtt in record.attachments:
                if recordAtt.esmapa:
                    result[recordAtt.id] = recordAtt.datas
                    _logger.info('**************')
        return result
    ...
    _columns = {
        ...
        'prueba': fields.function(_prueba, type="binary"),
